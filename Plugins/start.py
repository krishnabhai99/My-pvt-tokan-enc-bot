...
async def Handle_StartMsg(bot: Client, msg: Message):
    ...
    # Verification Handling (Fixed indentation and structure)
    if len(msg.command) > 1:
        data = msg.command[1]
        if data.split("-", 1)[0] == "verify":  # Corrected `if` structure
            userid = data.split("-", 2)[1]
            token = data.split("-", 3)[2]
            if str(msg.from_user.id) != str(userid):
            ...
            if is_valid:
                await msg.reply_text(
                    text=f"<b>Hey {msg.from_user.mention}, You are successfully verified!\nNow you have unlimited access for all files till today midnight.</b>",
                    protect_content=True
                )
...
async def Files_Option(bot: Client, message: Message):
    ...
    if VERIFY == "True" and not await check_verification(bot, message.from_user.id):
        btn = [[
    ...
    try:
        ...
        floodmsg = await message.reply_text(f"**😥 Please wait! Don't flood the bot, wait for {e.value} seconds.**", reply_to_message_id=message.id)
        await sleep(e.value)
...
async def cancel_process(bot: Client, message: Message):
    ...
    try:
        ...
        return await message.reply_text(text="**No ongoing process found!**")
    except Exception as e:
        print(f"Error while canceling process: {e}")
